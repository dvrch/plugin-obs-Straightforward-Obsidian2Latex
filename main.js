...
// src/converter/EmbeddedFileProcessor.ts
...
// src/converter/EmbeddedFileProcessor.ts
var EmbeddedFileProcessor = class {
  constructor(app) {
    this.app = app;
    this.pathManager = new PathManager(app);
  }
  extractEmbeddedReferences(content) {
    const references = [];
    ...
    while ((match = referenceRegex.exec(content)) !== null) {
      ...
      if (fileNameMatch) {
        ...
        const section = (sectionMatch == null ? void 0 : sectionMatch[1]) || "";
  ...
  async readEmbeddedFile(filePath) {
    try {
  ...
  async processEmbeddedReferences(content) {
    ...
    for (const reference of references) {
      ...
      if (fileContent) {
        ...
        if (sectionContent) {
          processedContent = processedContent.replace(reference.fullReference, `
% Start obsidian ref:
...
var LatexConverter = class {
  ...
  async convertNoteToLatex(file) {
    try {
      new import_obsidian3.Notice("D\xE9but de la conversion en LaTeX...");
      ...
      new import_obsidian3.Notice(`Conversion r\xE9ussie ! Fichier cr\xE9\xE9: ${texFilePath}`);
      if (this.settings.getAutoCompile()) {
        await this.compileLatexToPdf(texFilePath);
      }
    } catch (error) {
      console.error("Erreur lors de la conversion:", error);
      new import_obsidian3.Notice(`Erreur lors de la conversion: ${error.message}`);
    }
  }
  ...
  async compileLatexToPdf(texFilePath) {
    try {
      new import_obsidian3.Notice("Compilation LaTeX en cours...");
      if (!texFilePath) {
        const writingPath = this.pathManager.getWritingPath();
      ...
      if (result.success) {
        new import_obsidian3.Notice("Compilation r\xE9ussie ! PDF g\xE9n\xE9r\xE9.");
      } else {
        throw new Error(`Erreur de compilation: ${result.error}`);
      }
    } catch (error) {
      console.error("Erreur lors de la compilation:", error);
      new import_obsidian3.Notice(`Erreur lors de la compilation: ${error.message}`);
    }
  }
  async runCompleteTest() {
    try {
      new import_obsidian3.Notice("Lancement du test complet...");
      ...
      new import_obsidian3.Notice("Test complet r\xE9ussi !");
    } catch (error) {
      console.error("Erreur lors du test complet:", error);
      new import_obsidian3.Notice(`Erreur lors du test complet: ${error.message}`);
    }
  }
  getTexFilePath(file) {
    const baseName = this.pathManager.getBasename(file.path);
...
var NoteSelectionModal = class extends import_obsidian3.SuggestModal {
...
var LatexSettingsTab = class extends import_obsidian4.PluginSettingTab {
  ...
  createBasicSettingsSection(containerEl) {
    ...
    new import_obsidian4.Setting(containerEl).setName("Classe de document").setDesc("Classe LaTeX \xE0 utiliser").addDropdown((dropdown) => dropdown.addOption("extarticle", "Article \xE9tendu").addOption("article", "Article standard").addOption("ifacconf", "Conf\xE9rence IFAC").setValue(this.plugin.settings.getDocumentClass()).onChange(async (value) => {
      this.plugin.settings.updateSettings({ documentClass: value });
      await this.plugin.settings.saveSettings();
    }));
    new import_obsidian4.Setting(containerEl).setName("Taille de police").setDesc("Taille de police (ex: 12pt, 11pt)").addText((text) => text.setPlaceholder("12pt").setValue(this.plugin.settings.getFontSize()).onChange(async (value) => {
      this.plugin.settings.updateSettings({ fontSize: value });
      await this.plugin.settings.saveSettings();
    }));
    new import_obsidian4.Setting(containerEl).setName("Auteur").setDesc("Nom de l'auteur").addText((text) => text.setPlaceholder("Votre nom").setValue(this.plugin.settings.getAuthor()).onChange(async (value) => {
      this.plugin.settings.updateSettings({ author: value });
      await this.plugin.settings.saveSettings();
    }));
    new import_obsidian4.Setting(containerEl).setName("Titre").setDesc("Titre du document").addText((text) => text.setPlaceholder("Titre du document").setValue(this.plugin.settings.getTitle()).onChange(async (value) => {
      this.plugin.settings.updateSettings({ title: value });
      await this.plugin.settings.saveSettings();
    }));
    new import_obsidian4.Setting(containerEl).setName("Marges").setDesc("Marges de page (ex: 0.9in)").addText((text) => text.setPlaceholder("0.9in").setValue(this.plugin.settings.getMargin()).onChange(async (value) => {
  ...
  createParagraphSettingsSection(containerEl) {
    ...
    new import_obsidian4.Setting(containerEl).setName("Indentation premi\xE8re ligne").setDesc("Indentation en points (0 pour aucune)").addSlider((slider) => slider.setLimits(0, 50, 5).setValue(this.plugin.settings.getParagraphIndent()).setDynamicTooltip().onChange(async (value) => {
      this.plugin.settings.updateSettings({ paragraphIndent: value });
      await this.plugin.settings.saveSettings();
    }));
    new import_obsidian4.Setting(containerEl).setName("Table des mati\xE8res").setDesc("Ajouter une table des mati\xE8res").addToggle((toggle) => toggle.setValue(this.plugin.settings.getAddTableOfContents()).onChange(async (value) => {
      this.plugin.settings.updateSettings({ addTableOfContents: value });
      await this.plugin.settings.saveSettings();
    }));
    new import_obsidian4.Setting(containerEl).setName("Nouvelle page avant bibliographie").setDesc("Ins\xE9rer une nouvelle page avant la bibliographie").addToggle((toggle) => toggle.setValue(this.plugin.settings.getAddNewPageBeforeBibliography()).onChange(async (value) => {
      this.plugin.settings.updateSettings({ addNewPageBeforeBibliography: value });
      await this.plugin.settings.saveSettings();
    }));
    new import_obsidian4.Setting(containerEl).setName("Autoriser les sauts de page").setDesc("Autoriser les sauts de page dans les \xE9quations").addToggle((toggle) => toggle.setValue(this.plugin.settings.getAllowDisplayBreaks()).onChange(async (value) => {
  ...
  createFigureSettingsSection(containerEl) {
    ...
    new import_obsidian4.Setting(containerEl).setName("R\xE9duire l'espacement entre figures").setDesc("R\xE9duire l'espacement entre les figures").addToggle((toggle) => toggle.setValue(this.plugin.settings.getReduceSpacingBetweenFigures()).onChange(async (value) => {
      this.plugin.settings.updateSettings({ reduceSpacingBetweenFigures: value });
      await this.plugin.settings.saveSettings();
    }));
    new import_obsidian4.Setting(containerEl).setName("Figure sous le texte").setDesc("Placer les figures sous le texte").addToggle((toggle) => toggle.setValue(this.plugin.settings.getPutFigureBelowText()).onChange(async (value) => {
      this.plugin.settings.updateSettings({ putFigureBelowText: value });
      await this.plugin.settings.saveSettings();
    }));
    new import_obsidian4.Setting(containerEl).setName("Inclure le chemin").setDesc("Inclure le chemin complet des images").addToggle((toggle) => toggle.setValue(this.plugin.settings.getIncludePath()).onChange(async (value) => {
  ...
  createReferenceSettingsSection(containerEl) {
    ...
    new import_obsidian4.Setting(containerEl).setName("Convertir les r\xE9f\xE9rences non-embarqu\xE9es").setDesc("Convertir [[note]] en texte simple").addToggle((toggle) => toggle.setValue(this.plugin.settings.getConvertNonEmbeddedReferences()).onChange(async (value) => {
      this.plugin.settings.updateSettings({ convertNonEmbeddedReferences: value });
      await this.plugin.settings.saveSettings();
    }));
    new import_obsidian4.Setting(containerEl).setName("Traiter les blocs d'\xE9quations s\xE9par\xE9ment").setDesc("Traiter les blocs d'\xE9quations de mani\xE8re s\xE9par\xE9e").addToggle((toggle) => toggle.setValue(this.plugin.settings.getTreatEquationBlocksSeparately()).onChange(async (value) => {
      this.plugin.settings.updateSettings({ treatEquationBlocksSeparately: value });
      await this.plugin.settings.saveSettings();
    }));
    new import_obsidian4.Setting(containerEl).setName("Adapter la hi\xE9rarchie des sections").setDesc("Adapter la hi\xE9rarchie des sections dans les notes embarqu\xE9es").addToggle((toggle) => toggle.setValue(this.plugin.settings.getAdaptSectionHierarchy()).onChange(async (value) => {
      this.plugin.settings.updateSettings({ adaptSectionHierarchy: value });
      await this.plugin.settings.saveSettings();
    }));
    new import_obsidian4.Setting(containerEl).setName("Num\xE9ro de section apr\xE8s r\xE9f\xE9rence").setDesc("Ajouter le num\xE9ro de section apr\xE8s les r\xE9f\xE9rences internes").addToggle((toggle) => toggle.setValue(this.plugin.settings.getAddSectionNumberAfterReferencing()).onChange(async (value) => {
  ...
  createTableSettingsSection(containerEl) {
    ...
    new import_obsidian4.Setting(containerEl).setName("Package de tableau").setDesc("Package LaTeX pour les tableaux").addDropdown((dropdown) => dropdown.addOption("tabularx", "tabularx").addOption("longtable", "longtable").addOption("tabularray", "tabularray").setValue(this.plugin.settings.getTablePackage()).onChange(async (value) => {
      this.plugin.settings.updateSettings({ tablePackage: value });
      await this.plugin.settings.saveSettings();
    }));
    new import_obsidian4.Setting(containerEl).setName("Alignement des tableaux").setDesc("Alignement par d\xE9faut des tableaux").addDropdown((dropdown) => dropdown.addOption("center", "Centr\xE9").addOption("left", "Gauche").addOption("right", "Droite").setValue(this.plugin.settings.getTableAlignment()).onChange(async (value) => {
      this.plugin.settings.updateSettings({ tableAlignment: value });
      await this.plugin.settings.saveSettings();
    }));
    new import_obsidian4.Setting(containerEl).setName("Largeur relative des tableaux").setDesc("Largeur relative des tableaux (1.0 = 100%)").addSlider((slider) => slider.setLimits(0.5, 2, 0.1).setValue(this.plugin.settings.getTableRelWidth()).setDynamicTooltip().onChange(async (value) => {
  ...
  createCompilationSettingsSection(containerEl) {
    ...
    new import_obsidian4.Setting(containerEl).setName("Compilateur LaTeX").setDesc("Compilateur LaTeX \xE0 utiliser").addDropdown((dropdown) => dropdown.addOption("pdflatex", "pdflatex").addOption("xelatex", "xelatex").addOption("lualatex", "lualatex").setValue(this.plugin.settings.getLatexCompiler()).onChange(async (value) => {
      this.plugin.settings.updateSettings({ latexCompiler: value });
      await this.plugin.settings.saveSettings();
    }));
    new import_obsidian4.Setting(containerEl).setName("Compilation automatique").setDesc("Compiler automatiquement apr\xE8s conversion").addToggle((toggle) => toggle.setValue(this.plugin.settings.getAutoCompile()).onChange(async (value) => {
...
// src/main.ts
var StraightforwardObsidian2LatexPlugin = class extends import_obsidian5.Plugin {
  async onload() {
